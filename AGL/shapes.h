 ...
 *
 * The cube ranges from -1 to +1 on all the axises, ie. centered at the origin and going to unit length on all the
 * axises.
 */
 ...
 * \param calcNorm If \c true, also calculates the normals.
 *
 * Create a unit icosphere, ie. spehre created by subdividing a icosahedron. It has triangular faces. Read more [here](https://blender.stackexchange.com/questions/72/what-is-the-difference-between-a-uv-sphere-and-an-icosphere).
 *
 ...
 * mapped on the surface of a unit sphere. For example, a value of 0.5 will bring all the vertices halfways to the
 ...
 * but are promissed to be continuous. The coordinate for each vertex is generated by calculating the dot product of the
 * vertex with the respective parameter. The default parameters works almost well to generate non-repetative and smooth
 ...
 * the indices forming the faces (polygons) such that each trplet represents a non-overlapping triangle. This also means
 * that this algorithm will fail for non-convex polygons. The algorithm tries to equally distribute the vertices among
 * different faces to reduce crowding of edges. It also preserves the orientation of the face vertices in the new
 * triangular faces.
 */
