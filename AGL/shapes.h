...
Entity cuboid(float x=2, float y=2, float z=2, bool calcNorm=false);
Entity octahedron();
Entity dodecahedron();
Entity icosahedron();
Entity sphere(int lat=20, int lng=20, bool calcNorm=false);
Entity icosphere(int subdivision=3, bool calcNorm=false);
Entity cylinder(float r=1, float h=2, int strips=20);
Entity plane(float x=2, float z=2, bool calcNorm=false);
